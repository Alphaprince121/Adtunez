...
const DateRangeDropdown = () => {
    ...
    return (
        <div className="flex flex-col items-center mt-20" ref={dropdownRef}>
            {/* Dropdown Button */}
            <button
                onClick={toggleDropdown}
                className="flex items-center justify-start px-[12px] py-2 border font-medium text-[12px] leading-[20px] rounded-md text-[#898D9F]"
                aria-haspopup="true"
            ...
            {/* Main Dropdown */}
            {isDropdownOpen && (
                <div className="relative mt-2 w-72 bg-white border rounded-md shadow-lg z-10 p-4" role="menu">
                    <p className="text-[12px] leading-[16px] font-medium mb-2">Date Range</p>
                    <button
                        onClick={toggleMenu}
                        className="flex items-center text-[#333333] text-opacity-80 w-full justify-between font-medium text-[12px] leading-[16px] px-4 py-2 border rounded-md bg-white shadow-sm hover:bg-gray-100 mb-2"
                        aria-haspopup="true"
                    ...
                    {isMenuOpen && (
                        <div className="border-b pb-2 mb-2 relative group">
                            <div className=''>
                                {Object.keys(submenuItems).map((menu) => (
                                    <button
                                        key={menu}
                                        className="flex items-center justify-between px-4 py-2 hover:bg-gray-100 cursor-pointer rounded relative w-full font-[400] text-[12px] leading-[21px]"
                            ...
                            {activeSubmenu && (
                                <div className="absolute left-44 top-0 ml-2 w-48 bg-white border rounded-md shadow-lg" role="menu">
