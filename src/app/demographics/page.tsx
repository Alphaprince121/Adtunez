...
// Get min values for scaling
const minTotal = Math.min(...calculatedData.map(d => d.total));
const minAgeValues: Record<string, number> = calculatedData.reduce((acc, { gender, ageGroups }) => {
    acc[gender] = Math.min(...Object.values(ageGroups));
    return acc;
...
// Color Mapping
const colorScale = ["bg-indigo-100", "bg-indigo-300", "bg-indigo-500", "bg-indigo-700"];
const getColor = (gender: "Male" | "Female", value: number): string => {
    if (value === 0) return "bg-gray-100";
    const minAgeValue = minAgeValues[gender];
    return colorScale[Math.floor((value / minAgeValue) * (colorScale.length - 1))];
};
...
const GenderChart = () => (
    <div className="w-full -mt-3 relative">
        <div className="absolute border-r-2 border-[#898D9F] top-[11.5px] left-[17px] h-[76%] flex flex-col justify-around  pr-[5px]">
        ...
        <ResponsiveContainer width="100%" height={100}>
            <BarChart layout="vertical" data={calculatedData} margin={{ top: 10, right: 30, left: 65, bottom: 10 }}>
                <XAxis type="number" domain={[0, minTotal]} hide />
...
const GenderAgeChart = () => {
    ...
    return (
        <div className="flex relative flex-col items-center mt-4">
            {/* Vertical separator */}
            <div className="absolute border-r-2 border-[#898D9F] border-opacity-50 left-[56px] h-[76%] flex flex-col justify-around pr-[2px]"></div>

            {/* Grid Container */}
            <div className="grid-container grid grid-cols-[auto_repeat(8,minmin(2rem,1fr))] gap-1 items-center">
                {/* Male Section */}
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className="px-4 flex items-center justify-between py-2 rounded-md border border-gray-300 text-[#333333] font-medium text-[12px] leading-[20px] min-w-[90px] gap-5"
            >
                {selectedOption}
...
const Page = () => {
    ...
    return (
        <div className="min-w-[447px] mx-auto bg-white shadow-lg rounded-lg p-6 space-y-3 mt-20">
            <div className="flex justify-between items-center border-b-2 pb-2 relative">
                ...
                <div className="flex items-center gap-8">
                    ...
                    <img
                        ...
                        alt="three-dots"
                        className="h-5 cursor-pointer relative"
                ...
                <div className="flex border  rounded-lg p-1">
                    {["Gender", "Gender & Age", "Age"].map((tab) => (
                        <button
                            ...
                            className={`flex-1 px-4 py-2 rounded-md text-sm font-medium  ${activeTab === tab ? "border text-[#333333] font-semibold" : "text-gray-400"
                                }`}
                        >
                            {tab}</button>
                    ))}
            ...
            <div className="pt-2">
                ...
                <div className="flex text-indigo-700 items-center gap-1 cursor-pointer">
                    <h1 className="font-semibold text-sm underline">Demographics</h1>
